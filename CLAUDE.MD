...
5. **Запуск фонового воркера** — `await task_worker.start(bot)`:
   ...
   - `_FileCleanupService` создаётся в `TaskWorker.__init__` — директории и срок хранения читаются из `Config`
   - Запускает цикл `while running: task = await queue.dequeue()`
   - Запускает фоновый цикл очистки файлов (каждые 24 ч, файлы старше `FILE_MAX_AGE_DAYS` дней)
...
7. **Graceful shutdown** — строгий порядок освобождения ресурсов:
   - `await task_worker.stop()` — ждёт завершения активных задач, закрывает `AIComparator` (`await comparator.close()`), останавливает `_FileCleanupService`
...
UPLOAD_DIR=/root/progect/uploads       # Входные Excel/XML файлы от пользователей
DOWNLOAD_DIR=/root/progect/downloads   # Промежуточные файлы загрузки
OUTPUT_DIR=/root/progect/output        # Результаты синхронизации и отчёты
FILE_MAX_AGE_DAYS=7                    # Файлы старше этого срока удаляются уборщиком
...
```
aiogram3.4.1 asyncpg0.31.0 pandas2.3.3 openpyxl3.1.5 openai2.14.0 (для OpenRouter) python-dotenv1.2.1 aiohttp3.9.5 (для загрузки XML по URL) httpx0.28.1 (для прокси в AIComparator) redis7.4.0 aiofiles23.2.1 socksio==1.0.0 (для SOCKS-прокси)
```
...
[Unit]
Description=Marketplace Sync Telegram Bot
...
# 4. Зависимости
pip install aiogram==3.4.1 asyncpg==0.31.0 pandas==2.3.3 openpyxl==3.1.5 openai==2.14.0 python-dotenv==1.2.1 httpx==0.28.1 redis==7.4.0 aiohttp==3.9.5 aiofiles==23.2.1 socksio==1.0.0
...
# 9. Systemd сервис (см. секцию выше)
...
- Рекомендуемая версия Python: 3.12+ (тестировалось на 3.12.3)
...
- Временные файлы (uploads, downloads, output) хранятся на диске и удаляются
  фоновым уборщиком раз в 24 часа. Файлы старше `FILE_MAX_AGE_DAYS` (default=7 дней) удаляются автоматически.
...
6. Добавить фото-столбцы нового МП в `Config.PHOTO_COLUMNS`,
   `Config.PHOTO_READ_SEPARATORS`, `Config.PHOTO_WRITE_SEPARATORS`
7. Добавить метод `_sync_newmp_to_ozon` / `_sync_ozon_to_newmp`
   в `PhotoSyncer` (`services/sync/photo_syncer.py`)
...
**Версия документации**: 4.9
**Дата обновления**: Июль 2026
**Последние изменения**:
- v4.9: Три изменения в логике синхронизации. Габариты Яндекс: переход с составного столбца "Габариты с упаковкой, см" (формат «д/ш/в») на три раздельных столбца "Длина, см *", "Ширина, см *", "Высота, см *". Затронуты: DimensionsSynchronizer (чтение/запись), XmlSyncer (запись из XML), ColumnSyncer (удалён устаревший _write_yandex_composite), Config (маппинги и MANDATORY_MATCHES). Очистка ТН ВЭД: при синхронизации из Ozon в WB/Яндекс из значения ТНВЭД извлекается только числовой код ("2103909009 - Описание..." → "2103909009"). Настраивается через Config.TNVED_COLUMN_NAMES и Config.TNVED_NUMERIC_ONLY_MARKETPLACES. Реализовано в ColumnSyncer._apply_tnved_cleanup(). Принудительные парные сопоставления: добавлена конфигурация Config.FORCED_PAIR_ONLY_MATCHES и метод AIComparator._enforce_forced_pairs(). Два правила: Видео (WB+Яндекс, Ozon=NA) и НДС (WB+Ozon, Яндекс=NA). AI не может переопределить эти сопоставления — метод постобработки вычищает ошибочные тройные и парные, гарантирует правильное парное.
- v4.8: Оптимизация AI-валидации при сохранении файлов.
  `ExcelFileManager.save_results` теперь принимает опциональный `changes_log` и
  вызывает AI-проверку (`match_value_with_list`) ТОЛЬКО для ячеек, изменённых ботом
  при синхронизации. Ячейки, заполненные пользователем изначально, записываются
  без AI-запросов. Это сокращает количество AI-вызовов при сохранении с потенциальных
  сотен тысяч до нескольких сотен (только реально изменённые ячейки).
  `DataSynchronizer` передаёт `self.changes_log` в `save_results` автоматически.
- v4.7: Исправлен логгер — единый `logs/app.log` с `RotatingFileHandler` (10 МБ, 5 бэкапов)
  вместо нового файла `sync_YYYYMMDD_HHMMSS.log` при каждом вызове `setup_logger()`.
  Добавлен `AppLogger` — адаптер с поддержкой `trace_id` и `context`.
  Два форматтера: человекочитаемый в консоль (белый список ключей) и полный JSON в файл.
  Добавлены переменные окружения `UPLOAD_DIR`, `DOWNLOAD_DIR`, `OUTPUT_DIR`, `FILE_MAX_AGE_DAYS` в `Config`.
  Добавлен `_FileCleanupService` в `TaskWorker` — фоновая задача раз в 24 часа удаляет
  файлы старше `FILE_MAX_AGE_DAYS` дней из всех трёх директорий. Запускается и останавливается
  вместе с `TaskWorker`. Обход файловой системы выполняется в `run_in_executor`, не блокирует event loop.
