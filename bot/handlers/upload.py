...
from bot import storage  # ✅ Импортируем модуль для доступа к db
...
async def schema_selected(message: types.Message, state: FSMContext, bot) -> None:
    ...
    if schema_type == 'mvm':
        await state.update_data(selected_schema_id=schema_id, selected_schema_type='mvm', mvm_mp_files_processed=False)
        user_files[user_id] = {}
        await state.set_state(UploadMvmStates.waiting_for_mp_files)
        await message.answer(f"📦 МВМ-схема '{message.text}' выбрана\n\nОтправь 3 файла Excel (wb, ozon, yandex)\nЗатем — XML файл каталога", reply_markup=ReplyKeyboardRemove())
    else:
        await state.update_data(selected_schema_id=schema_id, selected_schema_type='standard')
        user_files[user_id] = {}
        await state.set_state(UploadStates.waiting_for_files)
        await message.answer(f"✅ Схема '{message.text}' выбрана\n\nОтправь 3 файла Excel", reply_markup=ReplyKeyboardRemove())
...
async def process_files(message: types.Message, state: FSMContext, bot) -> None:
    ...
    try:
        ...
        for marketplace, file_path in file_paths.items():
            await storage.db.add_file(user_id, processing_id, marketplace, os.path.basename(file_path), file_path)

        await message.answer("📖 Читаю файлы...")
        comparison_result = await storage.db.get_schema_matches(schema_id)
        await message.answer(f"🔄 Синхронизирую по схеме ({len(comparison_result.get('matches_all_three', []))} столбцов)...")
        ...
        if hasattr(synchronizer, 'ai_validation_log') and synchronizer.ai_validation_log:
            logger.info("📋 AI-логи: %d записей", len(synchronizer.ai_validation_log))
        ...
        await storage.db.complete_processing(processing_id, wb_count, ozon_count, yandex_count, total_synced)
        ...
        user_files[user_id] = {}
        await state.clear()
        await message.answer(f"✅ Готово!\n\n📦 Обработка товаров:\n• WB: {wb_count}\n• Ozon: {ozon_count}\n• Яндекс: {yandex_count}\n\n🔄 Синхронизировано ячеек: {total_synced}", reply_markup=get_main_menu_keyboard())
...
async def handle_mvm_upload_mp_file(message: types.Message, state: FSMContext, bot) -> None:
    ...
    file_path, file_name, marketplace = await download_file(bot, message, user_id)
    if not marketplace: await message.answer("❌ Переименуй файл (добавь wb/ozon/yandex)"); return
    if marketplace in user_files[user_id]: await message.answer(f"⚠️ {marketplace.upper()} уже загружен"); return
...
async def handle_mvm_upload_xml_file(message: types.Message, state: FSMContext, bot) -> None:
    ...
    if error == "file_too_big":
        ...
        await message.answer(f"⚠️ XML файл слишком большой ({file_size_mb} МБ).\nTelegram не позволяет скачивать файлы > 20 МБ.\n\n📎 Отправь прямую ссылку на XML файл\n(загрузи на Google Drive, Dropbox, Яндекс.Диск и скопируй прямую ссылку):\n\nИли нажми ❌ Отмена", reply_markup=get_mvm_waiting_xml_keyboard())
        return
    if error:
        await message.answer(f"❌ {error}\n\nОтправь файл заново: "); return
...
async def handle_mvm_upload_xml_text(message: types.Message, state: FSMContext) -> None:
    ...
    if text.startswith('http://') or text.startswith('https://'):
        ...
        if not url_filename.lower().endswith('.xml'): url_filename = "catalog.xml"

        xml_path, error = await download_file_by_url(text, user_id, url_filename)
        if error:
            await message.answer(f"❌ {error}\n\nПроверь ссылку и отправь заново, или отправь XML файл как документ: ", reply_markup=get_mvm_waiting_xml_keyboard())
            return
        await _validate_and_save_upload_xml(message, state, xml_path)
        return
...
async def _validate_and_save_upload_xml(message: types.Message, state: FSMContext, xml_path: str) -> None:
    try:
        ...
        await message.answer(f"❌ Ошибка чтения XML: {e}"); return
    except Exception as e:
        await message.answer(f"❌ Ошибка: {e}")
        logger.error("Ошибка XML при загрузке МВМ: %s", e, exc_info=True); return

    await state.update_data(mvm_xml_file_path=xml_path)
    await message.answer(f"✅ XML файл загружен! Офферов: {offer_count}\n\nНажми 🚀 Обработать для запуска синхронизации: ", reply_markup=get_process_keyboard())

async def process_files_mvm(message: types.Message, state: FSMContext, bot) -> None:
    user_id = message.from_user.id
    if user_id not in user_files or len(user_files[user_id]) != 3:
        await message.answer("❌ Загрузи 3 файла МП!"); return

    data = await state.get_data()
    schema_id = data.get('selected_schema_id')
    xml_file_path = data.get('mvm_xml_file_path')
    if not schema_id: await message.answer("❌ Схема не выбрана!"); return
    if not xml_file_path: await message.answer("❌ XML файл не загружен!"); return
    ...
    try:
        ...
        total_matches = sum(len(v) for k, v in comparison_result.items() if isinstance(v, list) and k.startswith('match'))
        await message.answer(f"🔄 Синхронизирую по МВМ-схеме\n📊 Сопоставлений: {total_matches}\n📦 XML офферов: {len(xml_offer_data)}")
        ...
        synchronizer = DataSynchronizer(
            comparison_result, ai_comparator=comparator,
        ...
        yandex_count = len(synced_dfs['yandex'])
        total_synced = sum(len(changes_log[mp]) for mp in changes_log)
        xml_filled = sum(1 for mp in changes_log for change in changes_log[mp] if change.get('source_marketplace') == 'xml')
        ...
        result_text = f"✅ Готово!\n\n📦 Обработка товаров:\n• WB: {wb_count}\n• Ozon: {ozon_count}\n• Яндекс: {yandex_count}\n\n🔄 Синхронизировано ячеек: {total_synced}"
        if xml_filled > 0: result_text += f"\n📦 Из XML каталога: {xml_filled}"
